...
export function ProfileInfoList({ profile }: { profile: Profile }) {
  ...
  const sections: { title: string; items: { label: string; value: string | null }[] }[] = [
    ...
    },
    {
      title: "学歴",
      items: [
        { label: "学校名", value: profile.school_name ?? null },
        { label: "学部・学科", value: profile.school_faculty ?? null },
      ],
    },
    {
      title: "職歴・スキル",
      items: [
        { label: "職歴", value: profile.work_history ?? null },
        { label: "スキル", value: profile.skills ?? null },
      ],
    },
    {
      title: "自己紹介",
      items: [{ label: "自己紹介", value: profile.self_introduction ?? null }],
    },
  ];
