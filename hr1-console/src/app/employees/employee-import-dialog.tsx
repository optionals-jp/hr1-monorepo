...
export function EmployeeImportDialog({
  ...
  const startImport = useCallback(async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      const hiringTypeMap: Record<string, string> = {
        新卒: "new_grad",
        中途: "mid_career",
        new_grad: "new_grad",
      ...
      try {
        ...
        if (v.current_address) body.current_address = v.current_address;
        if (v.registered_address) body.registered_address = v.registered_address;
        if (v.hiring_type && hiringTypeMap[v.hiring_type])
          body.hiring_type = hiringTypeMap[v.hiring_type];
        if (v.graduation_year) {
          const year = parseInt(v.graduation_year, 10);
  ...
  }, [validatedRows, organizationId, departments, onComplete]);
