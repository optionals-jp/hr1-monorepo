...
export default function NewFormPage() {
  ...
  const updateField = (tempId: string, field: string, value: string | boolean) => {
    setFields(
      fields.map((f) => (f.tempId === tempId ? { ...f, [field]: value } : f))
    );
  };
                ...
                <div key={field.tempId} className="rounded-lg border p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          v && updateField(field.tempId, "field_type", v)
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  {["radio", "checkbox", "dropdown"].includes(field.field_type) && (
                    <div className="space-y-1">
                      ...
                      <Textarea
                        value={field.options}
                        onChange={(e) =>
                          updateField(field.tempId, "options", e.target.value)
                        }
                        placeholder={"選択肢1\n選択肢2\n選択肢3"}
                        rows={3}
                      />
                  ...
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        updateField(field.tempId, "is_required", e.target.checked)
                      }
                      className="rounded"
                    />
                    必須項目
